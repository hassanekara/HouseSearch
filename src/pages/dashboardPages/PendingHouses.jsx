...
//     []
//   );
//   const filteredPendingDdata = rowData.filter((pendingHouse)=> pendingHouse.status === "pending");
...
//             <h2 className="text-2xl font-bold mb-4">PendingHouses</h2>
...
import Button1 from "../../components/Button1";
import { DELETE_MYHOUSE_DATA, GET_FILTERED_PENDIND_HOUSES_DATA, GET_FILTERED_UNRENTED_HOUSES_DATA } from '../../database/queries/MyHouseQueries';

const HouseTable = () => {
  const { loading, error, data } = useQuery(GET_FILTERED_PENDIND_HOUSES_DATA);
  ...
  return (
    ...
    <table>
      ...
      <tbody>
        {data.getFilteredPendingHouses.map((house) => (
          <tr key={house._id}>
            <td>{house._id}</td>
