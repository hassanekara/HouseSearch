...
import Button1 from "../../components/Button1";
import { DELETE_REQUEST_MUTATION } from '../../database/queries/RequestedUsers';
import { GET_ALL_OF_REQUESTED_DATA } from '../../database/queries/MoreHouseDedailsQueries';

const RequestedHouses = () => {
  const { loading, error, data } = useQuery(GET_ALL_OF_REQUESTED_DATA);
  const [deleteMyHouse] = useMutation(DELETE_REQUEST_MUTATION);

  if (loading) return <p>Loading...</p>;

  if (error) return <p>Error loading houses</p>;

  const handleDelete = async (deleteUserRequestId) => {
    try {
      await deleteMyHouse({ variables: { deleteUserRequestId }, refetchQueries: [{ query: GET_ALL_OF_REQUESTED_DATA }] });
      console.log(`House with ${deleteUserRequestId}  deleted successfully.`);
    } catch (error) {
  ...
  return (
    <div>
      <div className="flex justify-between items-center">
           <h2 className="text-2xl font-bold mb-4">All Houses Owner</h2>
           <Link to="/admin/add-new-house">
             <Button1 title="Add New House" icon="+" />
           </Link>
         </div>
    ...
    <table>
      ...
      <tbody>
        {data.getAllUsersRequested.map((houseOwner) => (
          <tr key={houseOwner._id}>
            {/* <td>{houseOwner._id}</td> */}
            <td>{houseOwner.fullName}</td>
            <td>{houseOwner.email}</td>
            <td>{houseOwner.telephone}</td>
            <td>{houseOwner.message}</td>
            <td>{houseOwner.house_id}</td>

            <td className='flex gap-4'>
              <FaTrash onClick={() => handleDelete(houseOwner._id)} style={{ cursor: 'pointer', marginRight: '10px' }} />
              <FaEye onClick={() => handleViewMore(houseOwner._id)} style={{ cursor: 'pointer' }} />
            </td>
          </tr>
        ))}
      </tbody>
...
export default RequestedHouses;
